...
void PlayerViewPanel::drawRays(int width, int height, int fov)
{
	float lineWidth = internalSize.X / rays.size();
	
	//draw floors
	
	float aspectRatio = internalSize.X / internalSize.Y;
	float near = 0;
	float far = map.height;
	...
	for (int r = 0; r < rays.size(); r++)
	{
		...
		int mapIndex = mapPos.Y * map.width + mapPos.X;
		Texture2D texture = map.textures[map.map[mapIndex]-1];

		float yPerTexturePixel = lineHeight / texture.Size.Y;
		...
		float xPerTexturePixel = map.squareSize / texture.Size.X;
		int wallX;
		int textX;
		if (ray.HitVertical)
		{
		...
		//draw walls
		for (int textY = textureYStartOffSet; textY < texture.Size.Y - textureYStartOffSet; textY++)
		{
			Vector2 texturePosition = Vector2(textX, textY);
			Color color = texture.GetPixel(texturePosition);
			/*color.R *= colorFactor;
			color.G *= colorFactor;
			color.B *= colorFactor;*/
			glColor3f(color.R, color.G, color.B);
